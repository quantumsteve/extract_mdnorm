...
public:
  ...
  void calculateIntersections(histogram &h, std::vector<float> &xValues, std::vector<Eigen::Vector3f> &intersections,
    ...
    xValues.clear();
    xValues.reserve(hNBins + kNBins + lNBins + 2);
    intersections.clear();
    ...
    if ((hEnd >= m_hX[0]) && (hEnd <= m_hX[hNBins - 1]) && (kEnd >= m_kX[0]) && (kEnd <= m_kX[kNBins - 1]) &&
      ...
      xValues.push_back(kfmax);
    }
    // sort intersections by final momentum
    //ranges::sort(ranges::views::zip(xValues, intersections),
    //             [](auto &&a, auto &&b) { return std::get<0>(a) < std::get<0>(b); });
    x86simdsort::keyvalue_qsort(xValues.data(),intersections.data(),xValues.size(),false);
  }
  ...
  template <typename histogram>
  void calcSingleDetectorNorm(const std::vector<Eigen::Vector3f> &intersections, const double &solid,
                              std::vector<double> &yValues, histogram &h) {
